...
export class FdnReverbProcessor {
  ...
  private readonly matrix: Float32Array; // row-major N×N, orthonormal
  private readonly feedback: Float32Array; // scratch: matrix·tail
  ...
  private velvetTaps: VelvetTap[];
  private readonly velvetBuffer: Float32Array;
  private velvetWrite = 0;
  private readonly velvetMask: number;
   ...
   */
  constructor(sampleRate: number, n = 8, rng: () => number = Math.random, delaysOverride?: number[]) {
    if (n !== 4 && n !== 8) {
  ...
  get velvetNoise(): readonly VelvetTap[] {
   ...
   * is a true bypass.
   */
  private diffuse(input: number, amount: number): number {
