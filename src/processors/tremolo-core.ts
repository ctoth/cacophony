...
export type ShapeMode = "sine" | "triangle" | "square";

/** Parameters consumed per processing block. */
export interface TremoloParams {
  /** LFO rate in Hz. */
  rate: number;
  /** Modulation depth in [0, 1]; 0 = bypass (g === 1), 1 = full 0..1 swing. */
  depth: number;
  /** LFO waveform. */
  shape: ShapeMode;
  /** Per-channel LFO phase offset in DEGREES (0 = mono, 90 = quadrature, 180 = auto-pan). */
  stereoPhase: number;
}
 ...
 */
export const TREMOLO_DEFAULTS: Readonly<TremoloParams> = {
  rate: 5,
  depth: 0.5,
  shape: "sine",
  stereoPhase: 0,
 ...
 */
export function lfoShape(phase: number, mode: ShapeMode): number {
...
export class TremoloProcessor {
   ...
   * so no zipper).
   */
  process(input: Float32Array, output: Float32Array, params: TremoloParams): void {
