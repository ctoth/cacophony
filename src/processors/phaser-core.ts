...
export interface PhaserParams {
  /** Center break frequency of the allpass sections, in Hz. */
  frequency: number;
  /** LFO frequency in Hz. */
  rate: number;
  /** Log sweep depth in octaves: break freq is multiplied by 2^(depth*lfo). */
  depth: number;
  /** Number of first-order allpass sections (2 per notch). Rounded to an int. */
  stages: number;
  /** Feedback (regeneration/resonance) gain on the cascade output. |fb| <= 0.95. */
  feedback: number;
  /** Additive wet gain on the cascade output (notch depth): y = x + mix*cascade. */
  mix: number;
}
 ...
 * over these.
 */
export const PHASER_DEFAULTS: Readonly<PhaserParams> = {
  frequency: 500,
  rate: 0.5,
  depth: 1.5,
  stages: 4,
...
export const MAX_PHASER_FEEDBACK = 0.95;
 ...
 */
export function breakFreqToAllpassCoeff(breakFreqHz: number, sampleRate: number): number {
 ...
 */
export function allpassStep(s: number, a: number, state: { z1: number }): number {
...
export class PhaserProcessor {
   ...
   */
  process(input: Float32Array, output: Float32Array, params: PhaserParams): void {
