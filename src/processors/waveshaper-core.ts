...
/**
 * The set of nonlinearity shapes the waveshaper can apply. Each shape ships a
 * matched (f, F0) pair where F0' = f and F0(0) = 0.
 */
export type WaveshaperShape = "hardclip" | "tanh";

/** Parameters consumed per processing block. */
export interface WaveshaperParams {
  /** Pre-gain (drive) applied to the input before the nonlinearity. >1 drives harder into saturation. */
  drive: number;
  /** Nonlinearity selection: "hardclip" (polynomial F0) or "tanh" soft clip (F0 = log cosh). */
  shape: WaveshaperShape;
  /** Wet/dry mix in [0,1]; 0 = dry (bypass), 1 = fully shaped. */
  mix: number;
  /** Post-nonlinearity output gain (linear). */
  output: number;
}
...
function fTanh(x: number): number {
  return Math.tanh(x);
}
 ...
 */
function f0Tanh(x: number): number {
  const ax = Math.abs(x);
 ...
 */
function fHardClip(x: number): number {
  if (x > 1) return 1;
  if (x < -1) return -1;
  return x;
}
 ...
 */
function f0HardClip(x: number): number {
  const ax = Math.abs(x);
  if (ax <= 1) return 0.5 * x * x;
  return ax - 0.5;
}

/** Resolve a shape name to its matched (f, F0) pair. */
function nonlinearity(shape: WaveshaperShape): { f: (x: number) => number; f0: (x: number) => number } {
  if (shape === "tanh") return { f: fTanh, f0: f0Tanh };
 ...
 */
export class WaveshaperProcessor {
  ...
  private cachedShape: WaveshaperShape = "hardclip";
   ...
   */
  process(input: Float32Array, output: Float32Array, params: WaveshaperParams): void {
 ...
 */
export function adaaSample(xn: number, xPrev: number, shape: WaveshaperShape, eps: number = ADAA_EPS): number {
...
export function shapeFunctions(shape: WaveshaperShape): { f: (x: number) => number; f0: (x: number) => number } {
