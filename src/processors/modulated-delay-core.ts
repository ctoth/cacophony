...
/** Fractional-delay interpolation order. Both are Lagrange FIR (Laakso 1996). */
export type InterpolationMode = "cubic" | "linear";

/** Parameters consumed per processing block. */
export interface ModulatedDelayParams {
  /** Nominal (center) delay in milliseconds — the fixed feedback tap center. */
  delayTime: number;
  /** Peak LFO delay excursion in milliseconds (CHORUS_WIDTH). 0 = pure delay. */
  depth: number;
  /** LFO frequency f_e in Hz. */
  rate: number;
  /** Feedback gain on the FIXED center tap. |feedback| <= 0.9999999 for stability. */
  feedback: number;
  /** Dry path gain (blend). */
  blend: number;
  /** Wet (modulated tap) gain (feedforward). */
  feedforward: number;
  /** Fractional-delay interpolation order (Lagrange FIR). */
...
export const DATTORRO_INV_SQRT2 = 0.7071;
 ...
 */
export const MODULATED_DELAY_DEFAULTS: Readonly<ModulatedDelayParams> = {
  delayTime: 5,
  depth: 0,
  rate: 0.5,
  feedback: 0,
...
export const MAX_DEPTH_MS = 50;
/** Stability bound on |feedback| (Dattorro Table 6, q23 max, p.776). */
export const MAX_FEEDBACK = 0.9999999;
 ...
 */
export function lagrangeLinear(d: number): [number, number] {
  return [1 - d, d];
}
 ...
 */
export function lagrangeCubic(d: number): [number, number, number, number] {
  const D = 1 + d;
  return [
    (-(D - 1) * (D - 2) * (D - 3)) / 6,
    (D * (D - 2) * (D - 3)) / 2,
 ...
 */
export class ModulatedDelayProcessor {
  /** Circular delay line. */
  ...
  private readInteger(delaySamples: number): number {
   ...
   */
  private readFractional(delaySamples: number, mode: InterpolationMode): number {
    ...
    return (
      h0 * this.readInteger(intDelay - 1) +
   ...
   */
  process(input: Float32Array, output: Float32Array, params: ModulatedDelayParams): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (readDelay > this.size - 3) readDelay = this.size - 3;
