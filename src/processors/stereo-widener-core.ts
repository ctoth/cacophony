export interface DecorrelatorTap {
  delay: number;
  gain: number;
}
...
export function buildDecorrelatorTaps(sampleRate: number, seed: number): DecorrelatorTap[] {
  const random = seededRandom(seed);
  const length = Math.max(32, Math.round(sampleRate * 0.02));
  const spacing = Math.max(3, Math.round(sampleRate / 2400));
  const taps: DecorrelatorTap[] = [];
  let energy = 0;
  ...
  for (const tap of taps) tap.gain *= norm;
  return taps;
}
...
class SparseFir {
  ...
  constructor(private readonly taps: DecorrelatorTap[]) {
  ...
  process(input: number): number {
    ...
    let output = 0;
    for (const tap of this.taps) output += tap.gain * this.buffer[(this.write - tap.delay) & this.mask];
...
export class StereoWidenerCore {
  private readonly leftFir: SparseFir;
  private readonly rightFir: SparseFir;
  private envelope = 0;
  private transient = 0;
  private readonly transientDecay: number;
  ...
  processSample(left: number, right: number, params: StereoWidenerParams): [number, number] {
    const level = Math.max(Math.abs(left), Math.abs(right));
    if (level > this.envelope * 3 + 0.04) this.transient = 1;
