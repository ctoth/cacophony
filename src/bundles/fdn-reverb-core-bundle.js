var fdnReverbCore = (function (exports) {
    'use strict';
    ...
    class FdnReverbProcessor {
        ...
        n;
        lines;
        matrix; // row-major N×N, orthonormal (U)
        fbDelays; // D_κ(z): per-path pure delays (DFM)
        velvets; // per-line VNS (Fagerström VFDN single)
        feedback; // scratch: matrix·delayed-tail
        scattered; // scratch: D_κ(z)·tail
         ...
         */
        constructor(sampleRate, n = 8, rng = Math.random, delaysOverride, feedbackDelaysOverride) {
            if (n !== 4 && n !== 8) {
        ...
        reset() {
            for (const line of this.lines)
                line.reset();
            for (const d of this.fbDelays)
                d.reset();
            for (const v of this.velvets)
                v.reset();
    ...
    exports.FdnReverbProcessor = FdnReverbProcessor;
    exports.buildHadamardMatrix = buildHadamardMatrix;
    exports.buildVelvetNoise = buildVelvetNoise;

    return exports;

})({});
//# sourceMappingURL=fdn-reverb-core-bundle.js.map
