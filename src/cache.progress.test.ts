...
describe("AudioCache Progress Tracking", () => {
  ...
  describe("Progress Tracking Tests (Should Fail Initially)", () => {
    ...
    it("should handle AbortSignal during progress tracking", async () => {
      ...
      // Start the request
      const requestPromise = cache.getAudioBuffer(audioContextMock, testUrl, abortController.signal, mockCallbacks);
    ...
    it("should deduplicate callbacks for concurrent requests", async () => {
      ...
      global.fetch = vi
        ...
        .mockImplementation(
          () =>
            new Promise((resolve) =>
              setTimeout(() => resolve(createMockResponse(mockArrayBuffer, { contentLength: 256 })), 20),
            ),
        );
